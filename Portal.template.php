...
function template_portal_above() {
	...
	<div class="sp_right sp_fullwidth">';
        ...
        if ($context['SPortal']['sides'][1]['active']) echo '
        ...
        if ($context['SPortal']['sides'][4]['active']) echo '
	...
	<div id="sp_header">';
        ...
        foreach ($context['SPortal']['blocks'][5] as $block) template_block($block);
    ...
    if (!empty($modSettings['showleft']) && !empty($context['SPortal']['blocks'][1])) {
        ...
        foreach ($context['SPortal']['blocks'][1] as $block) template_block($block);
    ...
    if (!empty($context['SPortal']['blocks'][2])) {
        foreach ($context['SPortal']['blocks'][2] as $block) template_block($block);

        if (empty($context['SPortal']['on_portal'])) echo '
...
function template_portal_below() {
    ...
    if (!empty($context['SPortal']['blocks'][3])) {
        if (empty($context['SPortal']['on_portal']) || !empty($context['SPortal']['blocks'][2]) || !empty($modSettings['articleactive'])) echo '
				<br class="sp_side_clear" />';

        foreach ($context['SPortal']['blocks'][3] as $block) template_block($block);
    }
    ...
    if (!empty($modSettings['showright']) && !empty($context['SPortal']['blocks'][4])) {
        ...
        foreach ($context['SPortal']['blocks'][4] as $block) template_block($block);
	...
	<div id="sp_footer">';
        ...
        foreach ($context['SPortal']['blocks'][6] as $block) template_block($block);
...
function template_block($block) {
    ...
    if (isset($txt['sp_custom_block_title_' . $block['id']])) $block['label'] = $txt['sp_custom_block_title_' . $block['id']];

    if ($context['SPortal']['core_compat']) template_block_core($block); else
...
function template_block_core($block) {
    ...
    if (empty($block['style']['no_title'])) {
        ...
        if (empty($block['force_view'])) echo '
								<a class="sp_float_right" href="javascript:void(0);" onclick="sp_collapseBlock(\'', $block['id'], '\')"><img id="sp_collapse_', $block['id'], '" class="icon" src="', $settings['images_url'], $block['collapsed'] ? '/expand.gif' : '/collapse.gif', '" alt="*" /></a>';
