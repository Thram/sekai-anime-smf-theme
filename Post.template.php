...
function template_main() {
    ...
    if ($context['browser']['is_firefox']) echo '
			function reActivate()
			{
    ...
    echo '
			var icon_urls = {';
    foreach ($context['icons'] as $icon) echo '
    ...
    if ($context['make_event'] && (!$context['event']['new'] || !empty($context['current_board']))) echo '
    ...
    if ($context['make_event']) {
        ...
        for ($year = $modSettings['cal_minyear']; $year <= $modSettings['cal_maxyear']; $year++) echo '
        ...
        for ($day = 1; $day <= $context['event']['last_day']; $day++) echo '
        ...
        if (!empty($modSettings['cal_allowspan']) || ($context['event']['new'] && $context['is_new_post'])) {
            ...
            if (!empty($modSettings['cal_allowspan'])) {
                ...
                for ($days = 1; $days <= $modSettings['cal_maxspan']; $days++) echo '
    ...
    // What about smileys?
    if (!empty($context['smileys']['postform']) || !empty($context['smileys']['popup'])) echo '
    ...
    if (isset($context['last_modified'])) echo '
    ...
    if ($context['can_post_attachment']) {
        ...
        if ($context['num_allowed_attachments'] > 1) echo '
							<script type="text/javascript"><!-- // --><![CDATA[
        ...
        if (!empty($modSettings['attachmentCheckExtensions'])) echo '
							', $txt['allowed_types'], ': ', $context['allowed_extensions'], '<br />';

        if (!empty($context['attachment_restrictions'])) echo '
							', $txt['attach_restrictions'], ' ', implode(', ', $context['attachment_restrictions']), '<br />';

        if (!$context['can_post_attachment_unapproved']) echo '
    ...
    if ($context['make_event'] && !$context['event']['new']) echo '
    ...
    if (isset($context['topic_last_message'])) echo '
    ...
    if (isset($context['previous_posts']) && count($context['previous_posts']) > 0) {
        ...
        foreach ($context['previous_posts'] as $post) {
            $ignoring = false;
            if (!empty($post['is_ignored'])) $ignored_posts[] = $ignoring = $post['id'];
